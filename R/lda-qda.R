...
qda.lda.info <- function(labeled_image1,labeled_image2,labeled_image3){
  # Trains and evalutes QDA and LDA models with three-fold cross-validation, to get the ROC curves for plotting
  #
  # Arguments:
  ...
  qda.roc1 = performance(prediction(qda1.pred$posterior[,2], labeled_image1$label), "tpr", "fpr")
  qda.roc2 = performance(prediction(qda2.pred$posterior[,2], labeled_image2$label), "tpr", "fpr")
  qda.roc3 = performance(prediction(qda3.pred$posterior[,2], labeled_image3$label), "tpr", "fpr")
  
  #AUC scores
  qda.auc1 = performance(prediction(qda1.pred$posterior[,2], labeled_image1$label), "auc")@y.values[[1]]
  qda.auc2 = performance(prediction(qda2.pred$posterior[,2], labeled_image2$label), "auc")@y.values[[1]]
  qda.auc3 = performance(prediction(qda3.pred$posterior[,2], labeled_image3$label), "auc")@y.values[[1]]
  ...
  #ROC values and ROC curves
  lda.roc1 = performance(prediction(lda1.pred$posterior[,2], labeled_image1$label), "tpr", "fpr")
  lda.roc2 = performance(prediction(lda2.pred$posterior[,2], labeled_image2$label), "tpr", "fpr")
  lda.roc3 = performance(prediction(lda3.pred$posterior[,2], labeled_image3$label), "tpr", "fpr")
  
  #AUC scores
  lda.auc1 = performance(prediction(lda1.pred$posterior[,2], labeled_image1$label), "auc")@y.values[[1]]
  lda.auc2 = performance(prediction(lda2.pred$posterior[,2], labeled_image2$label), "auc")@y.values[[1]]
  lda.auc3 = performance(prediction(lda3.pred$posterior[,2], labeled_image3$label), "auc")@y.values[[1]]
...
qda.confustion <- function(labeled_image1,labeled_image2,labeled_image3,method = "qda"){
  ...
  labeled_image1 = labeled_image1 %>% mutate(posterior = qda1.pred$posterior[,2]) %>% 
    mutate(confusion = ifelse(label == -1,ifelse(posterior < thresh,"TN","FN"),ifelse(posterior >= thresh,"TP","FP")))
  
  labeled_image2 = labeled_image2 %>% mutate(posterior = qda2.pred$posterior[,2]) %>% 
    mutate(confusion = ifelse(label == -1,ifelse(posterior < thresh,"TN","FN"),ifelse(posterior >= thresh,"TP","FP")))
  
  labeled_image3 = labeled_image3 %>% mutate(posterior = qda3.pred$posterior[,2]) %>% 
    mutate(confusion = ifelse(label == -1,ifelse(posterior < thresh,"TN","FN"),ifelse(posterior >= thresh,"TP","FP")))
